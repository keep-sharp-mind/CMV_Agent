...
1. `source_views` array: each item has `id`, `select` ("point" or "interval"), `source_channels` (array of channel names like ["x"]), `link_field` (data field name for linking)
2. `controlled_view` array: each item has `view` (target V-id), `field` (data field in target for linking), `action` ("filter" or "highlight")
3. The `link_field` in source and `field` in controlled_view must refer to the same underlying data concept for linking to work
...
{{
  "interaction_spec": {{
    "source_views": [{{"id": "V4", "select": "interval", "source_channels": ["x"], "link_field": "species"}}],
    "controlled_view": [{{"view": "V1", "field": "species", "action": "filter"}}]
  }},
...
- Is every non-d node (D, V, I) reachable from at least one other node?
